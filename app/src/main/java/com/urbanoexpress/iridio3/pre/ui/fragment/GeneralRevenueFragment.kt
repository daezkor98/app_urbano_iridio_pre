...
import com.urbanoexpress.iridio3.pre.databinding.FragmentGeneralRevenueBinding
import com.urbanoexpress.iridio3.pre.model.dto.CERT_ESTADO.APROBADO
import com.urbanoexpress.iridio3.pre.model.dto.Period
import com.urbanoexpress.iridio3.pre.presenter.viewmodel.GeneralRevenueViewModel
import com.urbanoexpress.iridio3.pre.ui.BaseActivity2
import com.urbanoexpress.iridio3.pre.ui.adapter.PeriodsRevenueAdapter
import com.urbanoexpress.iridio3.pre.ui.dialogs.FacturaPeriodoResumenDialog
...
class GeneralRevenueFragment : AppThemeBaseFragment() {
    ...
    private fun observeViewModel() {
        ...
        gananciasVM.generalRevenueDataLD.observe(this) {
            currentPeriod = it.data?.get(0)!!

            bind.tvWeekRevenue.text = "S/ ${currentPeriod.monto}"
            periodsAdapter.periods =
                it.data.filter { item ->
                    val monto = item.monto?.toDouble()
                    item.periodo!! > 0 && monto != 0.00 }
            areApproved =
    ...
    fun navigateToPeriodDetail(period: Period, isCurrent: Boolean) {
        closeDialog()
        val entregas = period.entregas?.toDouble()
        val noEntregas = period.no_entregas?.toDouble()
        //if (period.entregas == "0.00" && period.no_entregas == "0.00") {
        if (entregas == 0.00 && noEntregas == 0.00) {
