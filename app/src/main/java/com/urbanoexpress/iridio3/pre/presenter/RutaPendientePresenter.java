...
import com.urbanoexpress.iridio3.pre.model.interactor.ConsideracionesImportantesRutaInteractor;
...
import com.urbanoexpress.iridio3.pre.presenter.helpers.ActionMenuRutaPendienteHelper;
import com.urbanoexpress.iridio3.pre.presenter.helpers.ForzarCierreRutaHelper;
import com.urbanoexpress.iridio3.pre.ui.DetalleRutaRuralActivity;
...
import com.urbanoexpress.iridio3.pre.util.MyLocation;
    ...
    private void saveRutas(JSONArray jsonRutas) throws JSONException {
        ...
        if (dbRuta.size() > 0) {
            if (existeRutasPendientes(jsonRutas)) {
                ...
                ArrayList<JSONObject> rutasEstado = new ArrayList<>();
                for(int i = 0; i < jsonRutas.length(); i++){
                    rutasEstado.add(jsonRutas.getJSONObject(i));

                    if(rutasEstado.get(i).getInt("flag_valida_gestion") > 0){
                        JSONObject jsonRuta = jsonRutas.getJSONObject(i);
                        String idGuia = String.valueOf(jsonRuta.getInt("id_servicio"));

                        List<Ruta> existentes = Ruta.find(Ruta.class,
                                NamingHelper.toSQLNameDefault("idServicio") + " = ? and " +
                                        NamingHelper.toSQLNameDefault("estadoDescarga") + " = 20 ", idGuia);

                        if(existentes.isEmpty()){
                            List<Ruta> rutasDuplicadas = Ruta.find(Ruta.class,
                                    NamingHelper.toSQLNameDefault("idServicio") + " = ? and " +
                                            NamingHelper.toSQLNameDefault("estadoDescarga") + " = 10 ", idGuia);
                            for (Ruta ruta : rutasDuplicadas) {
                                ruta.delete();
                            }
                            saveRuta(jsonRuta);
                        }
...
//            }
    ...
    private void saveRuta(JSONObject jsonRuta) throws JSONException {
        ...
        Ruta ruta = new Ruta(//aqui se guarda la ruta en local
                ...
                flag_scaneo_pck,
                Integer.parseInt(jsonRuta.getString("flag_alerta")),
                jsonRuta.getInt("flag_valida_gestion") == 0 ? Ruta.EstadoDescarga.PENDIENTE : Ruta.EstadoDescarga.GESTIONADO,
                Ruta.ResultadoGestion.NO_DEFINIDO,
                Data.Delete.NO,
                Data.Validate.VALID,
                jsonRuta.getString("mensaje_custom_fotos"),
...
//            }
            String fechaRuta = fechaRutaActual == null
                    ? "2014-09-16" : fechaRutaActual;
            Date fechaActualRuta = new SimpleDateFormat("yyyy-MM-dd").parse(fechaRuta);
            Date fechaNuevaRuta = new SimpleDateFormat("yyyy-MM-dd").parse(
                    jsonGuias.getJSONObject(0).getString("fec_ruta"));
