...
import com.urbanoexpress.iridio3.pre.databinding.ModalRecoleccionSellerBinding;
import com.urbanoexpress.iridio3.pre.model.entity.Ruta;
import com.urbanoexpress.iridio3.pre.presenter.RecoleccionCounterPresenter;
...
import com.urbanoexpress.iridio3.pre.ui.adapter.ContenedoresRecoleccionAdapter;
import com.urbanoexpress.iridio3.pre.ui.adapter.GalleryAdapter;
import com.urbanoexpress.iridio3.pre.ui.adapter.GuiasRecolectadasAdapter;
import com.urbanoexpress.iridio3.pre.ui.adapter.PiezasRecolectadasAdapter;
import com.urbanoexpress.iridio3.pre.ui.adapter.RecolecioneGuiasSinAlistarAdapter;
import com.urbanoexpress.iridio3.pre.ui.adapter.model.GalleryWrapperItem;
import com.urbanoexpress.iridio3.pre.ui.widget.AdderButton;
...
public class RecoleccionCounterDialog  extends BaseDialogFragment implements RecoleccionCounterView,
        ...
        ContenedoresRecoleccionAdapter.OnContenedorRecoleccionListener {

    public static final String TAG = RecoleccionCounterDialog.class.getSimpleName();

    private ModalRecoleccionSellerBinding binding;
    private RecoleccionCounterPresenter presenter;

    public RecoleccionCounterDialog() {}
    ...
    public static RecoleccionCounterDialog newInstance(ArrayList<Ruta> rutas, int numVecesGestionado) {
        RecoleccionCounterDialog fragment = new RecoleccionCounterDialog();
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            presenter = new RecoleccionCounterPresenter(this,
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (!isAdded() || getActivity() == null) {
            com.google.firebase.crashlytics.FirebaseCrashlytics.getInstance().recordException(
                new IllegalStateException("RecoleccionCounterDialog.onActivityResult: dialog detached, requestCode=" + requestCode));
            return;
        }
    ...
    @Override
    public void showContenedores(List<ContenedorRecoleccionItem> items) {
        try {
            ContenedoresRecoleccionAdapter adapter = new ContenedoresRecoleccionAdapter(items, this);
            binding.piezasRecyclerView.setAdapter(adapter);
        } catch (NullPointerException ex) {
            ex.printStackTrace();
        }
    ...
    @Override
    public void onSelectionContenedorChanged(int position) {
        presenter.onSelectionPiezaChanged(position);
    }
    ...
    private void setupViews() {
        ...
        binding.moreActionsButton.setOnClickListener(v -> {
            ...
            popup.inflate(R.menu.menu_recoleccion_guia_electronica_counter);
            popup.setOnMenuItemClickListener(item -> {
                if (item.getItemId() == R.id.action_selection_mode_scan) {
                    presenter.onSelectionModeClick(RecoleccionCounterPresenter.SELECTION_MODE.SCAN);
                    return true;
                } else if (item.getItemId() == R.id.action_selection_mode_check) {
                    presenter.onSelectionModeClick(RecoleccionCounterPresenter.SELECTION_MODE.CHECK);
                    return true;
                } else if (item.getItemId() == R.id.collect_containers) {
                    presenter.onSelectionModeClick(RecoleccionCounterPresenter.SELECTION_MODE.CONTAINERS);
                    return true;
                }
                return false;
            });
