...
import com.urbanoexpress.iridio3.pre.view.ManifestarGuiaView;
...
public class ManifestarGuiaPresenter implements PiezasAdapter.OnPiezaListener {
    ...
    private void requestValidateGuia(String guia) {
        ...
        ApiService.getInstance().putParams("guia", guia);
        ApiService.getInstance().putParams("device_phone", Session.getUser().getDevicePhone());
        ApiService.getInstance().putParams("id_user", Session.getUser().getIdUsuario());
    ...
    private void requestManifestarGuia() {
        ...
        ApiService.getInstance().newParams();
        ApiService.getInstance().putParams("vp_piezas", piezasJSONArray.toString());
        ApiService.getInstance().putParams("vp_id_ruta", rutas.get(0).getIdRuta());
        ApiService.getInstance().putParams("device_phone", Session.getUser().getDevicePhone());
        ApiService.getInstance().putParams("id_user", Session.getUser().getIdUsuario());
        ApiService.getInstance().request(ApiRest.getInstance().getApiBaseUrl() +
