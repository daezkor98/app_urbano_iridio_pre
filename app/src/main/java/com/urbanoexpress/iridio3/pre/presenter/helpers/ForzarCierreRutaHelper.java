...
import com.urbanoexpress.iridio3.pre.databinding.ModalInputClaveForzarCierreRutaBinding;
...
public class ForzarCierreRutaHelper {
    ...
    private void validateCredencial(String username, String password) {
        if (CommonUtils.validateConnectivity(activity)) {
            ...
            ApiService.getInstance().putParams("username",      username);
            ApiService.getInstance().putParams("password",      CommonUtils.getSHA1(password));
            ApiService.getInstance().putParams("imei",          Session.getUser().getDevicePhone());
            ApiService.getInstance().putParams("vp_id_user",    Preferences.getInstance().getString("idUsuario", ""));
