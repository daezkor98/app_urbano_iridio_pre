...
import com.urbanoexpress.iridio3.pre.databinding.ActivityForgotPasswordBinding;
...
import com.urbanoexpress.iridio3.pre.util.network.Connection;
...
//    }
    ...
    private void validateUserName() {
        if (Connection.hasNetworkConnectivity(ForgotPasswordActivity.this)) {
            ...
            ApiService.getInstance().putParams("user_name", binding.txtUser.getText().toString().trim());
            ApiService.getInstance().putParams("device_imei", devicePhone);
            ApiService.getInstance().request(ApiRest.getInstance().getApiBaseUrl() + ApiRest.Api.VALIDATE_USER_NAME_COURIER,
...
//    }
    ...
    private void changeUserPassword() {
        if (Connection.hasNetworkConnectivity(ForgotPasswordActivity.this)) {
            showProgressDialog();
            ApiService.getInstance().newParams();
            ApiService.getInstance().putParams("id_user", idUser);
            ApiService.getInstance().putParams("new_password",
                    CommonUtils.getSHA1(binding.txtRepeatPassword.getText().toString().trim()));
            ApiService.getInstance().putParams("device_imei", devicePhone);
            ApiService.getInstance().request(ApiRest.getInstance().getApiBaseUrl() + ApiRest.Api.CHANGE_USER_COURIER_PASSWORD,
                    ApiService.TypeParams.FORM_DATA, new ApiService.ResponseListener() {
                        @Override
                        public void onResponse(JSONObject response) {
                            dismissProgressDialog();
