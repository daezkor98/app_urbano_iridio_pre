...
import com.urbanoexpress.iridio3.pre.ui.adapter.model.GalleryButtonItem;
import com.urbanoexpress.iridio3.pre.ui.adapter.model.GalleryPhotoItem;
import com.urbanoexpress.iridio3.pre.ui.adapter.model.GalleryWrapperItem;
import com.urbanoexpress.iridio3.pre.ui.model.ContenedorRecoleccionItem;
...
import com.urbanoexpress.iridio3.pre.util.ValidationUtils;
import com.urbanoexpress.iridio3.pre.util.constant.LocalAction;
import com.urbanoexpress.iridio3.pre.view.RecoleccionCounterView;
...
public class RecoleccionCounterPresenter {

    private static final String TAG = RecoleccionCounterPresenter.class.getSimpleName();

    private RecoleccionCounterView view;
    private ArrayList<Ruta> rutas;

    private List<PiezaRecolectadaItem> piezasRecolectadasItems = new ArrayList<>();
    private List<PiezaRecolectadaItem> guiasMalEmbaladasItems = new ArrayList<>();
    private List<ContenedorRecoleccionItem> contenedorRecoleccionItems = new ArrayList<>();
    ...
    private boolean continueWithoutSelectGuias = false;
    ...
    public RecoleccionCounterPresenter(RecoleccionCounterView view, ArrayList<Ruta> rutas,
    ...
    public void onConfirmNoSelectedGuias() {
        continueWithoutSelectGuias = true;
        showNextStepFromStepGuias();
    }
    ...
    public void onSelectionModeClick(SELECTION_MODE selectionMode) {
        ...
        contenedorRecoleccionItems.clear();
        ...
        if (selectionMode == SELECTION_MODE.SCAN) {
            ...
            view.showPiezasRecolectadas(piezasRecolectadasItems);
                ...
                selectionMode == SELECTION_MODE.CONTAINERS) {
            ...
            if (selectionMode == SELECTION_MODE.CHECK) {
                view.showGuiasRecolectadas(piezasRecolectadasItems);
                onLoadGuiasElectronicasClick();
            } else {
                view.showContenedores(contenedorRecoleccionItems);
                onLoadContenedoresClick();
            }
        }
        view.setTextTotalRecolectados(getTotalPiezasRecolectadas() + " recolectados");
    }
    ...
    public void onSelectAllGuiasCheckedChange(boolean isChecked) {
        if (currentStep == STEPS.GUIAS) {
            ...
            if (contenedorRecoleccionItems.size() > 0) {
                contenedorRecoleccionItems.stream().forEach(item -> item.setSelected(isChecked));
                view.notifyPiezasAllItemChanged();
                view.setTextTotalRecolectados(getTotalPiezasRecolectadas() + " recolectados");
            }
        } else if (currentStep == STEPS.GUIAS_MAL_ENBALADAS) {
            if (guiasMalEmbaladasItems.size() > 0) {
                guiasMalEmbaladasItems.stream().forEach(item -> item.setSelected(isChecked));
                view.notifyPiezasAllItemChanged();
            }
        }
    }
    ...
    public void onBtnSiguienteClick() {
        if (currentStep == STEPS.GUIAS) {
            if (selectionMode == SELECTION_MODE.SCAN) {
                if (validateSelectedGuias()) {
                    view.setVisibilityStepGuias(false);
                    view.setVisibilityStepFormulario(true);
                ...
                if (validateSelectedGuias()) {
                    ...
                    view.showDialogConfirmNoSelectedGuias();
                }
            } else if (selectionMode == SELECTION_MODE.CONTAINERS) {
                if (validateSelectedGuias()) {
                    view.setVisibilityStepGuias(false);
                    view.setVisibilityStepFormulario(true);
                    currentStep = STEPS.FORMULARIO;
        ...
        if (currentStep == STEPS.GUIAS_MAL_ENBALADAS) {
            view.setVisibilityStepGuiasMalEmbaladas(false);

            List<PiezaRecolectadaItem> guiasSinAlistarItems = guiasMalEmbaladasItems.stream()
                    .filter(item -> !item.isSelected())
                    .map(PiezaRecolectadaItem::new)
                    .collect(Collectors.toList());

            if (guiasSinAlistarItems.size() > 0) {
                view.setVisibilityStepGuiasSinAlistar(true);
                view.showGuiasSinAlistar(guiasSinAlistarItems);
                view.setTextBottomMessage(
                        view.getViewContext().getString(R.string.bottom_msg_select_guias_sin_alistar));
                view.setVisibilityBottomMsg(true);
                currentStep = STEPS.GUIAS_SIN_ALISTAR;
            } else {
                view.setVisibilityStepFormulario(true);
                view.setVisibilityBottomMsg(false);
                currentStep = STEPS.FORMULARIO;
            }
            return;
        }

        if (currentStep == STEPS.GUIAS_SIN_ALISTAR) {
            view.setVisibilityBottomMsg(false);
            view.setVisibilityStepGuiasSinAlistar(false);
            view.setVisibilityStepFormulario(true);
            currentStep = STEPS.FORMULARIO;
            return;
        }
        ...
        if (currentStep == STEPS.FORMULARIO) {
            if (validateFormularioRecoleccion()) {
                view.setVisibilityStepFormulario(false);
                view.setVisibilityStepFotosProducto(true);
        ...
        if (currentStep == STEPS.FOTOS_PRODUCTO) {
            if (validateFotosProducto()) {
                view.setVisibilityStepFotosProducto(false);
                view.setVisibilityStepFirmaCliente(true);
        ...
        if (currentStep == STEPS.FIRMA_CLIENTE) {
            view.setVisibilityStepFirmaCliente(false);
            view.setVisibilityStepFotosCargo(true);
        ...
        if (currentStep == STEPS.FOTOS_CARGO) {
            if (validateFotosCargo()) {
                view.setVisibilityStepFotosCargo(false);
                view.setVisibilityStepFotosDomicilio(true);
    ...
    public void onLoadContenedoresClick() {
        if (CommonUtils.validateConnectivity(view.getViewContext())) {
            view.showProgressDialog(R.string.text_cargando_guias_electronicas);
            ...
            final RequestCallback callback = new RequestCallback() {
                ...
                public void onSuccess(JSONObject response) {
                    try {
                        ...
                        if (response.getBoolean("success")) {
                            handleDataContenedores(response.getJSONArray("data"));
                            if (contenedorRecoleccionItems.size() > 0) {
                                view.showContenedores(contenedorRecoleccionItems);
                            } else {
                                view.showToast(R.string.activity_detalle_ruta_msg_recoleccion_no_hay_contenedores);
                            }
                            view.setTextTotalRecolectados(getTotalPiezasRecolectadas() + " recolectados");
                        } else {
                            view.showToast(response.getString("msg_error"));
            ...
            RutaPendienteInteractor.getContenedoresRecoleccion(params, callback);
        }
    }
                ...
                try {
                    ...
                    if (response.getBoolean("success")) {
                        ...
                        PiezaRecolectadaItem pieza = new PiezaRecolectadaItem(
                                ...
                                "",
                                data.getString("tipo_barra").equals("WYB") ? PiezaRecolectadaItem.Type.WYB : PiezaRecolectadaItem.Type.PCK,
                                true
                        );
    ...
    private void handleDataGuiasElectroncias(JSONArray data) throws JSONException {
        for (int i = 0; i < data.length(); i++) {
            ...
            piezasRecolectadasItems.add(new PiezaRecolectadaItem(
                    ...
                    jsonObject.getString("fecha_ss"),
                    PiezaRecolectadaItem.Type.WYB,
                    false
            ));
        }
    }

    private void handleDataContenedores(JSONArray data) throws JSONException {
        for (int i = 0; i < data.length(); i++) {
            JSONObject jsonObject = data.getJSONObject(i);
            contenedorRecoleccionItems.add(new ContenedorRecoleccionItem(
                    jsonObject.getString("des_id"),
                    jsonObject.getString("des_codigo"),
                    jsonObject.getString("guias"),
                    jsonObject.getString("piezas"),
                    false
            ));
        }
    ...
    private void setVisibilityFirstStep() {
        ...
        view.setVisibilityStepGuias(true);
        view.showPiezasRecolectadas(piezasRecolectadasItems);
        view.setEnabledFormPaquete(false);
        view.setEnabledFormSobre(false);
        view.setEnabledFormValija(false);
    ...
    private void showNextStepFromStepGuias() {
        boolean isAllSelected = piezasRecolectadasItems.stream().allMatch(PiezaRecolectadaItem::isSelected);
        ...
        guiasMalEmbaladasItems = piezasRecolectadasItems.stream()
                .filter(item -> !item.isSelected())
                .filter(item -> {
                    LocalDate fechaSS;
                    try {
                        fechaSS = LocalDate.parse(item.getFechaSS(),
                                DateTimeFormatter.ofPattern("dd/MM/yyyy"));
                    } catch (DateTimeParseException ex) {
                        fechaSS = LocalDate.now();
                    }
                    return fechaSS.isBefore(localDateNow);
                })
                .map(PiezaRecolectadaItem::new)
                .collect(Collectors.toList());
        ...
        if (guiasMalEmbaladasItems.size() > 0) {
            view.setVisibilityStepGuias(false);
            view.setVisibilityStepGuiasMalEmbaladas(true);
            view.showGuiasRecolectadas(guiasMalEmbaladasItems);
            view.setTextBottomMessage(
                    view.getViewContext().getString(R.string.bottom_msg_select_guias_mal_embaladas));
            view.setVisibilityBottomMsg(true);
            currentStep = STEPS.GUIAS_MAL_ENBALADAS;
        } else {
            view.setVisibilityStepGuias(false);
            view.setVisibilityStepFormulario(true);
            currentStep = STEPS.FORMULARIO;
    ...
    private void saveGestionGE(String fecha, String hora) {
        ...
        int totalGuias = piezasRecolectadasItems.size();

        for (int i = 0; i < rutas.size(); i++) {
            if (totalGuias == 0 || continueWithoutSelectGuias) {
    ...
    private void saveGestionContenedor(String fecha, String hora) {
        Log.d(TAG, "SAVE CONTENEDOR: " + rutas.size());
        int totalGuias = contenedorRecoleccionItems.size();

        for (int i = 0; i < rutas.size(); i++) {
            for (int j = 0; j < totalGuias; j++) {
                if (contenedorRecoleccionItems.get(j).isSelected()) {
                    GuiaGestionada guiaGestionada = new GuiaGestionada(
                            Preferences.getInstance().getString("idUsuario", ""),
                            rutas.get(i).getIdServicio(),
                            "168",
                            ...
                            String.valueOf(LocationUtils.getLongitude()),
                            "", "", "",
                            contenedorRecoleccionItems.get(j).getPiezas(),
                            "0",
                            String.valueOf(GuiaGestionada.Recoleccion.CONTENEDOR),
                            contenedorRecoleccionItems.get(j).getBarra(),
                            "",
                            "",
                            view.getTextComentarios(),
                            "0", "",
    ...
    private void saveGuiasNoRecolectadas(String fecha, String hora) {
        Log.d(TAG, "SAVE FORMULARIO GUIAS NO RECOLECTADAS: " + guiasMalEmbaladasItems.size());

        for (int j = 0; j < guiasMalEmbaladasItems.size(); j++) {
            String motivoId = guiasMalEmbaladasItems.get(j).isSelected() ? "188" : "185";
            GuiaGestionada guiaGestionada = new GuiaGestionada(
                    Preferences.getInstance().getString("idUsuario", ""),
                    rutas.get(0).getIdServicio(),
                    motivoId,
                    ...
                    String.valueOf(LocationUtils.getLongitude()),
                    "", "", "",
                    guiasMalEmbaladasItems.get(j).getPiezas(),
                    "0",
                    String.valueOf(GuiaGestionada.Recoleccion.GUIA_ELECTRONICA),
                    guiasMalEmbaladasItems.get(j).getBarra(),
                    "",
                    "",
                    view.getTextComentarios(),
                    "0", "",
    ...
    private int getTotalPiezasRecolectadas() {
        ...
        if (selectionMode == SELECTION_MODE.SCAN) {
            ...
            totalPiezas = contenedorRecoleccionItems.stream().mapToInt(item -> {
                if (item.isSelected()) {
                    try {
                        return Integer.parseInt(item.getPiezas());
                    } catch (NumberFormatException ex) { }
    ...
    private boolean validateSelectedGuias() {
        if (selectionMode == SELECTION_MODE.SCAN) {
            ...
            if (!contenedorRecoleccionItems.stream().anyMatch(ContenedorRecoleccionItem::isSelected)) {
                view.showSnackBar(R.string.activity_detalle_ruta_msg_seleccione_contenedor);
                CommonUtils.vibrateDevice(view.getViewContext(), 100);
                return false;
            }
        }
    ...
    private boolean existPiezaRecolectada(PiezaRecolectadaItem pieza) {
        if (pieza.getType() == PiezaRecolectadaItem.Type.PCK) {
    ...
    private boolean validateFormularioRecoleccion() {
        ...
        if (!(view.getValueFormPaquete() > 0 || view.getValueFormSobre() > 0
                || view.getValueFormValija() > 0 || view.getValueFormOtros() > 0)) {
    ...
    private void updateEstadoGestionGE(int procesoDescarga) {
        for (int i = 0; i < rutas.size(); i++) {
            ...
            if (descargaRuta != null) {
                descargaRuta.setProcesoDescarga(procesoDescarga);
                descargaRuta.save();
            }
    ...
    private class SaveGestionTask extends AsyncTaskCoroutine<String, String> {
        ...
        public String doInBackground(String... strings) {
            ...
            if (selectionMode == SELECTION_MODE.SCAN ||
                    selectionMode == SELECTION_MODE.CHECK) {
                saveGestionGE(fecha, hora);
                saveGuiasNoRecolectadas(fecha, hora);
            } else {
                saveGestionContenedor(fecha, hora);
            }
