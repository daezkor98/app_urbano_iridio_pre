...
import com.urbanoexpress.iridio3.pre.view.VerficationCodeView;
...
public class VerficationCodePresenter {
    ...
    private void requestValidateVerificationEmail(String email) {
        if (Connection.hasNetworkConnectivity(view.getViewContext())) {
            ...
            ApiService.getInstance().putParams("email", email);
            ApiService.getInstance().putParams("device", Build.MODEL);
            ApiService.getInstance().putParams("version", Build.VERSION.RELEASE);

            ApiService.getInstance().request(
                    ApiRest.getInstance().getApiBaseUrl() + ApiRest.Api.VALIDATE_VERIFICATION_EMAIL,
